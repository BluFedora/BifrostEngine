...
namespace bf
{
  ...
  {
    return TextEncodingResult<TextEncoding::UTF32_LE>{bfBytesReadUint32LE(reinterpret_cast<const char*>(characters)), characters + 1};
  }

  TextEncodingResult<TextEncoding::UTF32_BE> utf32BECodepoint(const CodeUnit<TextEncoding::UTF32_BE>* characters) noexcept
  {
    return TextEncodingResult<TextEncoding::UTF32_BE>{bfBytesReadUint32BE(reinterpret_cast<const char*>(characters)), characters + 1};
  }
