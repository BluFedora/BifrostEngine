  ...
  class Light final : public Component<Light>
  {
   ...
   private:
    struct LightGPUDataCache
    {
      ...
      bool  is_dirty;               //!< If the data needs to be uploaded to the GPU
    };
   ...
   private:
    ...
    float             m_InnerAngleRad;   //!< For SPOT, must be less than [Light::m_OuterAngleRad] and greater than 0.0f.
    float             m_OuterAngleRad;   //!< For SPOT, must be greater than [Light::m_InnerAngleRad] and less than Two PI.
    LightGPUDataCache m_GPUCache;        //!< For POINT and SPOT, A cache of some calculations needed for shading.
