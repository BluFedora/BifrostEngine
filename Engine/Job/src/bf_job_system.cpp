...
#elif IS_POSIX
#include <unistd.h>  // also MACOS 10.5+ not 10.4
#else
#include <sys/param.h>
...
namespace bf
{
  namespace job
  {
    ...
    struct JobSystem
    {
      ...
      {
        worker_sleep_cv.notify_one();
      }

      void wakeUpAllWorkers()
      {
        worker_sleep_cv.notify_all();
      }
    };
    ...
    struct ThreadWorker
    {
      ...
      // The Queues memory footprint can be reduced
      ...
      TaskHandle                                      num_allocated_tasks = 0u;
    ...
    void ThreadWorker::garbageCollectAllocatedTasks(const WorkerID thread_id)
    {
      if (num_allocated_tasks)
      {
        ...
        const auto done_end = std::partition(allocated_tasks_bgn, allocated_tasks_end, [this, thread_id](TaskHandle task_hdl) {
          Task* const task = taskPtrToPointer(TaskPtr{thread_id, task_hdl});

          return !taskIsDone(task);
        });

        for (auto it = done_end; it != allocated_tasks_end; ++it)
        {
          Task* const task = taskPtrToPointer({thread_id, *it});

          assert(task->owning_worker == thread_id);
